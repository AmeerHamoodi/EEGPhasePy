...
def test_estimator_get_phase_at_triggers():
   trigger_param_sets = [
      ...
      [np.sin(2 * np.pi * 5 * np.arange(0, 2, 1/500)), [238, 121, 221], True, True],
      [np.sin(2 * np.pi * 5 * np.arange(0, 2, 1/500)), [238, 121, 221], False, True],
   ]
      ...
      if check._is_array(trigger_param_outputs[i]) and (trigger_param_outputs[i][0] == ValueError or trigger_param_outputs[i][0] == TypeError):
         ...
         for j, true_phase in enumerate(trigger_param_outputs[i]):
            if true_phase <= 2*np.pi:
                assert np.abs(phase[j]) == pytest.approx(true_phase, abs=0.1)
            else:
...
def test_estimator_compute_phase_stats():
    phase_param_sets = [
       [np.sin(2 * np.pi * 5 * np.arange(0, 10, 1/500)), [125, 225, 425, 625, 525, 825]],
       [np.sin(2 * np.pi * 5 * np.arange(0, 2, 1/500)) + np.random.normal(0, 1, size=1000), [125, 225, 325]],
    ]
    ...
    for i, param_set in enumerate(phase_param_sets):
       assert estimator.mean_phase_from_triggers(param_set[0], param_set[1], toDegree=True) == pytest.approx(trigger_param_outputs[i][0], abs=6)
       assert estimator.std_phase_from_triggers(param_set[0], param_set[1], toDegree=True) == pytest.approx(trigger_param_outputs[i][1], abs=15)